...
namespace Tauron.Application.Ioc.BuildUp.Strategy.DefaultStrategy
{
    ...
    public class MethodInjector : MemberInjector
    {
        ...
        {
            entry?.BuildActions.Add(c => Inject(Argument.CheckResult(c.Target, nameof(c.Target)), c.Container, metadata, interceptor, c.ErrorTracer, c.Parameters, null));
            ...
            foreach (var parameterInfo in parms.Select(p => new ParameterMemberInfo(p)))
                new ParameterHelper(_metadataFactory, parameterInfo, args, _resolverExtensions).Inject(target, container, metadata, interceptor, errorTracer, parameters, null);
        ...
        private class ParameterHelper : Injectorbase<ParameterMemberInfo>
        {
            ...
            protected override Action<object, object> GetInjectAction() => throw new NotSupportedException();
